-- Tighten the already-deployed group-assignment RPC without rewriting migration
-- 0025. Invited students may be placed before first sign-in, while completed
-- and archived groups are never valid assignment targets.
...
begin
  ...
  if not found then
    raise exception 'Only an active or invited student profile can be assigned to a group.';
  end if;
  ...
  if not found then
    raise exception 'Only planned or active groups can receive students.';
  end if;
